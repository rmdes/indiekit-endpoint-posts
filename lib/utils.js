  ...
  for (const line of yamlString.split("\n")) {
    ...
    if (kvMatch) {
      ...
      if (value === "") {
        ...
        // Strip quotes
        frontmatter[currentKey] = value.replaceAll(/^["']|["']$/g, "");
      }
    } else {
      isInArray = false;
    }
...
export const importPostFromFile = async (url, publication, application) => {
  try {
    ...
    const contentDirectory = publication.store?.options?.directory;
    if (!contentDirectory) {
      console.log(
        "[import] FAIL: no contentDirectory. store=%o, store.options=%o",
    ...
    const absolutePath = path.join(contentDirectory, relativePath);
    ...
    const postType = directoryToPostType[directoryName] || "note";
